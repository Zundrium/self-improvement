...
>
	{#each $toastStore as item (item.id)}
		<article
			...
			class={`pointer-events-auto flex touch-pan-y items-center gap-4 rounded-3xl px-5 py-4 text-(--status-foreground) ${toastColor(item.type)}`}
		...
		>
			...
			<div class="min-w-0 flex-1">
				...
				{#if item.description}
					<p class="mt-0.5 text-sm leading-5 text-(--status-foreground)">{item.description}</p>
				{/if}
				{#if item.action}
