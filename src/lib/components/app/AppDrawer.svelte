...
import { formatScreenTime, formatBedtime, formatUsageSeconds } from '$lib/trackers/formatting';
import TrackerTile from '$lib/components/tracker/TrackerTile.svelte';
...
const trackerDetails = $derived({
	...
	nutrition: {
		value: daySummary.nutritionFasting ? 'Fasting' : `${daySummary.calories.toLocaleString()} kcal`,
	...
	stretch: trackerDetail(
		daySummary.stretchScheduled ? (daySummary.stretchDone ? 'Completed' : 'Not yet') : 'Rest day',
	...
	<div class="mx-auto w-full max-w-(--app-compact-max-width)">
		...
		{:else}
			<section class="space-y-4 py-8 text-center">
				...
				<p class="text-sm text-(--text-muted)">Choose the trackers you want to use in your profile.</p>
				<Button profile="highlighted" size="medium" href="/profile" onclick={onSelect}>Choose trackers</Button>
			</section>
		{/if}
	</div>
</div>
