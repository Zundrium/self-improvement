	...
	<div class="mx-auto w-full max-w-2xl space-y-6">
		...
		<Card>
			...
			<CardContent class="space-y-4">
				...
				<Button size="medium" type="button" profile="plain" disabled={Boolean(busy)} onclick={openAppSettings}>
		...
		<Card>
			...
			<CardContent class="space-y-5">
					...
					{#each TRACKER_IDS as tracker (tracker)}
						<div class="flex items-center justify-between gap-4 py-3 first:pt-0 last:pb-0">
							<div class="min-w-0">
								...
								<p class="text-xs leading-5 text-(--text-muted)">{trackerDescription(tracker)}</p>
							</div>
				...
				<div class="flex flex-wrap gap-2">
					...
					<Button size="medium" type="button" profile="plain" disabled={Boolean(busy)} onclick={openUsageAccess}>
						<Smartphone class="size-4" /> Usage access
					</Button>
					<Button size="medium" type="button" profile="plain" disabled={Boolean(busy)} onclick={refreshAccess}>
			...
			<CardFooter>
				<Button size="medium"
					class="w-full"
					type="button"
					profile={allGranted ? 'highlighted' : 'plain'}
