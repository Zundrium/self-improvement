...
let { date, workout, audioManager, completed, ontoggle, onspeedchange }: Props = $props();
	...
	<TrackerSection ariaLabel="Workout details" class="mx-auto max-w-3xl">
		<div class="space-y-4">
			<div>
				...
				<p class="mt-2 max-w-2xl text-sm leading-6 text-(--text-muted)">{workout.description}</p>
			</div>
			...
			<div class="space-y-1">
				{#each workout.activities as activity, index (activity.id)}
					<div class="flex items-center gap-3 py-2">
						<span class="w-5 text-center text-xs text-(--text-muted)">{index + 1}</span>
