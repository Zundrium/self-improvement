<script lang="ts">
	...
	async function analyzePhoto() {
		...
		try {
			const result = await api<{ estimate: MealEstimate }>('/calories/api/meals/analyze', {
				method: 'POST',
	...
	async function submitCorrection(event: SubmitEvent) {
		...
		try {
			const result = await api<{ estimate: MealEstimate }>('/calories/api/meals/refine', {
				method: 'POST',
				body: JSON.stringify({
					image: selectedImage,
					estimate,
	...
	{:else if phase === 'analyzing' || phase === 'refining'}
		<header class="border-b border-(--text)/8 bg-(--bg)/90 backdrop-blur-xl">
			<div class="mx-auto flex min-h-16 max-w-5xl items-center justify-between gap-3 px-4 sm:px-6">
				...
				<Button href="/calories/log/{data.date}" variant="ghost" size="icon" aria-label="Cancel"
