<script lang="ts">
	...
	async function postCompletion(completion: BreathingCompletion) {
		const response = await fetch(resolve('/breathing'), {
			method: 'POST',
			headers: { 'content-type': 'application/json' },
			body: JSON.stringify(completion)
		});
		if (!response.ok) throw new Error('Could not save breathing exercise');
		return (await response.json()) as SavedExercise;
	}
