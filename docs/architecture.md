...
- Route-owned action rules use `defineActionCandidate`. Eligibility belongs in composable `conditions`; attributed trackers use `trackerIds`; extra enabled-tracker dependencies use `requires`. The factory owns required-tracker merging, the default tracker icon, and stable instance IDs. `trackerActionCandidates` explicitly registers every app tracker with an unconditional `.status` baseline candidate. The selector keeps one card per enabled tracker: contextual actions retain priority/score/conflict ordering, and baselines fill all remaining trackers. Complete/rest/attention baselines suppress inappropriate activity prompts; warnings still take precedence. Completed and rest cards sort last and use neutral surfaces, with a checkmark only for completion. Feed status is a presentation projection, not a new stored completion or gamification record. The registry classifies Steps, Sleep and Screen time as passive; other app trackers are active. This classification only breaks tied baseline scores, without changing contextual priorities.
